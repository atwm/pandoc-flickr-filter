...
### Changed
- `transformBlocks` now accepts a `width :: T.Text` parameter, allowing callers
  to control the CSS `width` attribute applied to generated images.
- The `pandoc-flickr-filter` executable reads `flickr-width` from document
  metadata (e.g. `flickr-width: 90%` in YAML front matter) and falls back to
  `"100%"` when the field is absent.

## [0.1.0.2] - 2026-05-07

### Changed
- Generated `Figure` blocks now carry `style="text-align: center;"` so that
  photos are centred in HTML output.
